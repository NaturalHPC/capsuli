...
Intel refers to hwthreads as "logical processors", hwloc calls them "processing units"
or PUs and so does OpenMPI unless it uses the term hwthread just to confuse things a bit
more. Note that Linux also uses the term "logical processor" for anything its scheduler
assigns processes to, which is a hardware thread if SMT is available but is a whole core
if it isn't.
...
On modern non-embedded operating systems, hardware access is mediated by the operating
system, and we're mainly concerned with Linux here because that is what all the clusters
are running (see the note on macOS below). Information about the CPU(s) can be obtained
on Linux from the /proc/cpuinfo file, or equivalently but more modernly, from the files
in /sys/devices/system/cpu/cpu<x>/topology/.
...
Capsuli uses logical hwthread ids everywhere, it does not use physical ids.

Linux knows how hwthreads are grouped into bigger things of course. Cores are identified
in Linux using the "core id", which is listed in /proc/cpuinfo and in
...
Capsuli does not use core ids, although it uses groups of hwthread ids that contain all
the hwthreads for a given core.


Resource binding

Running processes need something to run on, a hwthread. The assignment of a process to
...
moving a process from one hwthread to another takes time. In Capsuli, when running on a
cluster, each process is assigned its own specific set of hwthreads to run on, and we
try to bind it to the assigned hwthreads.
...
MPICH doesn't support binding, as far as I can see, and Capsuli does not support it yet.
...
```
Capsuli     hwthread        logical hwthread id         physical hwthread id
...
class Core:
    ...
    def __ior__(self, other: object) -> "Core | NotImplementedType":
        if not isinstance(other, Core):
            return NotImplemented
    ...
    def __isub__(self, other: object) -> "Core | NotImplementedType":
        if not isinstance(other, Core):
            return NotImplemented
...
class CoreSet:
    ...
    def __ior__(self, other: object) -> "CoreSet | NotImplementedType":
        if not isinstance(other, CoreSet):
            return NotImplemented
    ...
    def __isub__(self, other: object) -> "CoreSet | NotImplementedType":
        if not isinstance(other, CoreSet):
            return NotImplemented
    ...
    def isdisjoint(self, other: "CoreSet") -> bool:
        """Returns whether we share resources with other."""
        for cid, core in self._cores.items():
            if cid in other._cores and not core.isdisjoint(other._cores[cid]):  # noqa: SLF001
                return False
        return True
    ...
    def get_first_cores(self, num_cores: int) -> "CoreSet":
        ...
        Args:
            num_cores: The number of cores to select.
        """
        result = copy(self)
        ...
        result._cores = {c.cid: c for c in result._cores.values() if c.cid in selected}  # noqa: SLF001
        return result
...
class OnNodeResources:
    ...
    def __ior__(self, other: object) -> "OnNodeResources | NotImplementedType":
        if not isinstance(other, OnNodeResources):
            return NotImplemented
    ...
    def __isub__(self, other: object) -> "OnNodeResources | NotImplementedType":
        if not isinstance(other, OnNodeResources):
            return NotImplemented
    ...
    def __str__(self) -> str:
        return f"OnNodeResources({self.node_name}, c: {self.cpu_cores!s})"

    def __repr__(self) -> str:
        return f'OnNodeResources("{self.node_name}", {self.cpu_cores!r})'
...
class Resources:
    ...
    def __ior__(self, other: object) -> "Resources | NotImplementedType":
    ...
    def __isub__(self, other: object) -> "Resources | NotImplementedType":
    ...
    def isdisjoint(self, other: "Resources") -> bool:
        ...
        for node_name, node in self._nodes.items():
            if node_name in other._nodes and not node.isdisjoint(  # noqa: SLF001
                other._nodes[node_name]  # noqa: SLF001
            ):
                return False
        return True
