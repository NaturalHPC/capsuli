...
from capsuli._impl.resources import Resources
...
def test_core_equals(c1: Core) -> None:
    c2 = Core(0, {0, 1})
    c3 = Core(1, {0, 1})
    c4 = Core(0, {2, 3})

    assert c1 == c2
    assert not c1 != c2  # noqa: SIM202
    assert c1 != c3
    assert c1 != c4
    assert c3 != c4


def test_core_length(c1: Core) -> None:
    assert len(c1) == 2
...
def test_core_copy(c1: Core) -> None:
    c2 = copy(c1)
    assert c2.cid == 0
...
def test_core_union() -> None:
    ...
    c3 = Core(2, {2})
    with pytest.raises(ValueError, match="merge hwthreads on different cores"):
        c1 | c3


def test_core_union_onto(c1: Core) -> None:
    c2 = Core(0, {2, 3})
...
def test_core_isdisjoint(c1: Core) -> None:
    c2 = Core(0, {0})
    c3 = Core(0, {2, 3})
    c4 = Core(1, {0, 1})
    ...
    with pytest.raises(ValueError, match="compare hwthreads on different cores"):
        c1.isdisjoint(c4)


def test_core_str(c1: Core) -> None:
    assert str(c1) == "0(0,1)"


def test_core_repr(c1: Core) -> None:
...
def test_core_set_equals(cs1: CoreSet) -> None:
    ...
    assert cs1 == cs2
    assert not cs1 != cs2  # noqa: SIM202
    assert cs1 != cs3
    assert cs1 != cs4
    assert cs1 != cs5
    assert cs1 != cs6
    assert not cs3 == cs4  # noqa: SIM201
    assert cs4 != cs5


def test_core_set_length(cs1: CoreSet) -> None:
    cs2 = CoreSet([])
...
def test_core_set_iter(cs1: CoreSet) -> None:
...
def test_core_set_copy(cs1: CoreSet) -> None:
    cs2 = copy(cs1)
    assert cs1 == cs2
...
def test_core_set_union_onto(cs1: CoreSet) -> None:
...
def test_core_set_subtract_disjunct(cs1: CoreSet) -> None:
    cs2 = CoreSet([Core(3, {6, 7})])
    cs1 -= cs2
...
def test_core_set_subtract_whole_core(cs1: CoreSet) -> None:
...
def test_core_set_subtract_threads(cs1: CoreSet) -> None:
...
def test_core_set_str(cs1: CoreSet) -> None:
    assert str(cs1) == "0-1(0-3)"


def test_core_set_repr(cs1: CoreSet) -> None:
    assert repr(cs1) == "CoreSet({Core(0, {0,1}), Core(1, {2,3})})"


def test_core_set_get_first_cores(cs1: CoreSet) -> None:
...
@pytest.fixture
def n1(cs1: CoreSet) -> OnNodeResources:
    return OnNodeResources("node001", cs1)


def test_node_resources_equals(n1: OnNodeResources) -> None:
...
def test_node_resources_copy(n1: OnNodeResources) -> None:
    n2 = copy(n1)
...
def test_node_resources_union_onto(n1: OnNodeResources) -> None:
    ...
    with pytest.raises(ValueError, match="merge resources on different nodes"):
        n1 |= n4


def test_node_resources_hwthreads(n1: OnNodeResources) -> None:
    assert list(n1.hwthreads()) == [0, 1, 2, 3]


def test_node_resources_subtract(n1: OnNodeResources) -> None:
    n2 = OnNodeResources("node001", CoreSet([Core(0, {0, 1}), Core(4, {8, 9, 10, 11})]))
    ...
    with pytest.raises(ValueError, match="remove resources on different nodes"):
        n1 -= n4


@pytest.fixture
def r1(n1: OnNodeResources) -> Resources:
    return Resources([n1])


def test_resources_length(r1: Resources, n1: OnNodeResources) -> None:
...
def test_resources_iter(cs1: CoreSet, n1: OnNodeResources) -> None:
...
def test_resources_equals(r1: Resources) -> None:
    assert r1 == Resources(
...
def test_resources_subtract(r1: Resources) -> None:
    r2 = Resources([])
    r2 -= r1
...
def test_resources_total_cores() -> None:
    r1 = Resources(
        [
...
def test_resource_hwthreads(n1: OnNodeResources, r1: Resources) -> None:
...
def test_resources_isdisjoint(r1: Resources) -> None:
    r2 = Resources([])
...
def test_resources_union(r1: Resources) -> None:
    r2 = Resources([])
    r3 = Resources([OnNodeResources("node001", CoreSet([Core(0, {0})]))])
    r4 = Resources([OnNodeResources("node001", CoreSet([Core(0, {2})]))])
    r5 = Resources([OnNodeResources("node002", CoreSet([Core(0, {0})]))])
